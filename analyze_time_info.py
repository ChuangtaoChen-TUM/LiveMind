...
total_gen_time = [[] for _ in range(num_files)]
...
gen_time_per_category = [{} for _ in range(num_files)]
gen_time_per_len = [{} for _ in range(num_files)]
...
for i in range(num_files):
    for entry in data[i]:
        ...
        total_gen_time[i].append(entry["time_info"]["total_gen_time"])
        ...
        gen_time_per_category[i][category].append(entry["time_info"]["total_gen_time"])
        gen_time_per_len[i][current_len].append(entry["time_info"]["total_gen_time"])


# check if categories and lengths are consistent across files
...
# save time info by category
with open(output_dir+'/timeinfo_by_category.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    row_title = ["Category", "Total"]\
              + [f"File {i+1} Accuracy" for i in range(num_files)]\
              + [f"File {i+1} Latency" for i in range(num_files)]\
              + [f"File {i+1} Gen Time" for i in range(num_files)]
    writer.writerow(row_title)

    row_all = ["All", len(data[0])]\
            + [100*total_acc[i]["correct"]/total_acc[i]["count"] for i in range(num_files)]\
            + [np.mean(total_latency[i]) for i in range(num_files)]\
            + [np.mean(total_gen_time[i]) for i in range(num_files)]
    ...
    for category in categories:
        ...
        row = [category, total]\
            ...
            + [np.mean(gen_time_per_category[i][category]) for i in range(num_files)]
        writer.writerow(row)

# save time info by length
with open(output_dir+'/timeinfo_by_len.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    row_title = ["Length", "Total"]\
              + [f"File {i+1} Accuracy" for i in range(num_files)]\
              + [f"File {i+1} Latency" for i in range(num_files)]\
              + [f"File {i+1} Gen Time" for i in range(num_files)]
    writer.writerow(row_title)
    ...
    for length in lengths:
        ...
        row = [length, total]\
            ...
            + [np.mean(gen_time_per_len[i][length]) for i in range(num_files)]
        writer.writerow(row)
