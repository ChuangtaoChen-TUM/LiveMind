""" python ./mmlu_pro_analyze_batched.py ./output/batched/.../all.json, this will create a report in the same directory as the input file """
import json
import numpy as np
import csv
import argparse
...
for i in range(num_files):
    for entry in data[i]:
        ...
        total_latency[i].append(entry["usage"]["completion_tokens"][-1])
        latency_per_category[i][category].append(entry["usage"]["completion_tokens"][-1])
        latency_per_len[i][current_len].append(entry["usage"]["completion_tokens"][-1])
...
with open(output_dir+'/timeinfo_by_category.csv', 'w', newline='') as file:
    ...
    row_title = ["Category", "Total"] + [f"File {i+1} Accuracy" for i in range(num_files)] + [f"File {i+1} Tokens" for i in range(num_files)]
...
with open(output_dir+'/timeinfo_by_len.csv', 'w', newline='') as file:
    writer = csv.writer(file)
    row_title = ["Length", "Total"] + [f"File {i+1} Accuracy" for i in range(num_files)] + [f"File {i+1} Tokens" for i in range(num_files)]
    writer.writerow(row_title)
    row_all = ["All", len(data[0])] + [100*total_acc[i]["correct"]/total_acc[i]["count"] for i in range(num_files)] + [np.mean(total_latency[i]) for i in range(num_files)]
    writer.writerow(row_all)
